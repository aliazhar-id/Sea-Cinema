...
@section('body')
  <div class="w-full h-1/2 flex flex-col relative">
    @php
      $backDropPath = $movieData ? "{$imageBaseURL}/original{$movieData->backdrop_path}" : '';
    @endphp

    {{-- Background Image --}}
    <img src="{{ $backDropPath }}" alt="" class="h-1 absolute object-cover lg:object-fill">
    <div class="w-full h-1 absolute bg-black bg-opacity-60 z-10"></div>

    {{-- Menu Section --}}
    <div class="w-full bg-transparent h-[96px] drop-shadow-lg flex row items-center z-10" 96px="">
      <div class="w-1/3 pl-5">
        <a href="/movies"
          class="uppercase text-base mx-5 text-white hover:text-movieapp-500 duration-200 font-inter">pilm</a>
      </div>
    ...
    {{-- Content Section --}}
    <div class="w-1/2 h-1/2 z-10 flex flex-col justify-center px-20">
      <span class="font-quicksand font-bold text-6xl mt-4 text-white">tetxt{{ $title }}</span>
